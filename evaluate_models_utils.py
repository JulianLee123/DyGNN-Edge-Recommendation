...
def evaluate_model_link_recommendation(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader, evaluate_data: Data, edge_raw_features: np.ndarray, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000, use_classification_metrics = False, only_need_predicts = False):
    """
    ...
    all_predicts = [] # Only setup for non-classify version: no need to gather for the other
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            batch_ref_src_node_ids, batch_ref_dst_node_ids, batch_ref_node_interact_times, batch_ref_edge_ids = \
                evaluate_data.ref_src_node_ids[evaluate_data_indices], evaluate_data.ref_dst_node_ids[evaluate_data_indices], \
                evaluate_data.ref_node_interact_times[evaluate_data_indices], evaluate_data.ref_edge_ids[evaluate_data_indices]
            ...
            if use_classification_metrics:
                ...
                evaluate_y_trues.append(batch_labels)
                evaluate_y_predicts.append(pred_labels)
                all_predicts.append(predicts.cpu().numpy()[:,0])
            else:
    ...
    if use_classification_metrics and not only_need_predicts:
    ...
    return evaluate_losses, evaluate_metrics, all_predicts
